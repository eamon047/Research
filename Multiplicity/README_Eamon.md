...
这份文件用于说明当前 `Multiplicity/` 的工程角色、本地运行约定，以及它与
已归档原始 release、`LibKGE/` 和 `Research/` 文档的分工。
...
- 说明当前 `Multiplicity/` 相对于已归档原始 release 的定位
- 记录本地环境下可直接运行的分析脚本入口
- 记录当前仍然有效的运行约定与方法口径
...
当前本地维护与 thesis 分析的工作目录，原名为 `Multiplicity_rewrite/`。
...
因此，`Multiplicity/` 之后的主要任务不再是不断扩新的 relation pattern，
而是：
...
### 5.3 当前本地主实验不是论文的严格 epsilon-level set 实现

这点仍然必须牢记。

当前 `Multiplicity/main.py` 保留了 multiplicity 主计算逻辑，但它不等于
论文中的严格流程：
...
- 如果后面继续做参数收缩、committee 缩减或其他计算量控制实验，
  这份文件仍应作为 `Multiplicity/` 的工程说明入口

## 12. 当前一句话总结

`Multiplicity/` 现在的角色，是 thesis 主线分析与后续工程维护的本地工作目录；
