...
def calc_performance(o_scores, s_scores, mask, mn_mask, dataset_dir):
    """
...
def evaluation(model, dataset, aggregation_func, args):
    ...
    for agg_path_list in rep_path_list:
        ...
        performance_list.append(
            calc_performance(o_agg_scores, s_agg_scores, rel_mask, mn_mask, dataset_dir)
        )
...
def batch_evaluation():
    parser = argparse.ArgumentParser(description="Evaluate top-1 query-answering multiplicity.")
    ...
    save_name = args.output_dir / f"multi_rep{args.num}_agg{args.agg}_random{args.seed}"
