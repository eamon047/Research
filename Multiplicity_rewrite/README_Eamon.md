...
- `Research/thesis_inverse_experiment.md`
  记录 inverse 实验设置、过程与阶段性结果
- `Multiplicity_rewrite/README_Eamon.md`
  记录代码入口、目录结构、运行口径与工程注意事项

这样后续补 `TransE` 或新增数据集时，不需要反复改理论定义文件，只需更新实验记录与代码说明。

## 当前 inverse 分析代码与结果位置

当前 inverse family 的第一步代码也已经单独拉出，位于：
...
- 只基于训练图计算 inverse-related 结构统计
- 不加载 8 个 repeated runs 的模型参数
- 不做 multiplicity 推理
- 只输出 relation-level inverse stats 与 pair-level inverse scores

`inverse_analysis.py` 的职责是：

- 将 inverse stats 与 relation-level multiplicity 表 merge
- 输出 merged CSV
- 计算 Spearman 相关性
- 计算 inverse-strength 分桶统计

`inverse_mapping_interaction_analysis.py` 的职责是：

- 在 merged inverse table 上按 `mapping_type` 分层
- 分别计算各类 relation 内部的 inverse-strength 相关性
- 输出按 mapping type 分层的 bucket 统计
...
当前第一步 sanity check 已经完成，且第一轮 merge analysis 也已经完成。

首先，结构统计本身是可用的：

- inverse strength 的分布没有塌缩
- relation-level inverse feature 不是全 0 变量
- top-scoring partner 多数具有合理的 reverse structural support

当前 inverse strength 的分布概况为：
...
- `inverse_strength = 0` 的 relation 数：`97`
- `inverse_strength >= 0.5` 的 relation 数：`34`
- `inverse_strength >= 0.8` 的 relation 数：`12`

因此当前判断是：

- inverse family 这一特征值得继续推进
- 但其与 multiplicity 的关系暂时不是简单单调关系

当前 merge analysis 的主要现象是：

- `inverse_strength` 与 `alpha_r / delta_r` 的 Spearman 相关在当前 combined relation-level setting 下为弱正相关
- `inverse_strength` 与 `hits_r` 为弱负相关
- 但 bucket analysis 显示真正的模式更像非单调结构：
  - `inverse_strength = 0` 的 relation 和 `inverse_strength > 0.5` 的 relation 都相对更稳定
  - 中间强度区间反而更容易表现出更低的 `Hits@10` 和更高的 multiplicity

因此当前更稳的判断是：

- inverse family 这一特征仍然值得继续推进
- 但后续应该优先做可视化与交互项检查，而不是过早把它写成“inverse 越强 multiplicity 越低”

进一步的 `mapping_type` 交互分析显示：

- `1-N` 和 `M-1` 中，`inverse_strength` 与 `alpha / delta` 呈更明显的正相关
- `M-N` 中，这种相关性明显更弱
- `1-1` 方向相反，但样本太少，不宜强解释

所以当前最合理的口径是：

- inverse 的作用不是全局统一的
- 它会受到 relation mapping structure 的调制
